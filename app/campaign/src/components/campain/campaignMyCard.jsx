import { Link } from 'react-router-dom'
import { formatDollar } from '@tbook/utils/lib/conf'

export default function CampaignCard ({
  campaignId,
  picUrl,
  name,
  nft,
  points,
  projectId,
  participantNum = 0
}) {
  return (
    <Link
      to={`/app/${projectId}/campaign/${campaignId}`}
      target='__blank'
      className='block rounded-xl bg-[rgb(144,75,246)]/[0.1] p-5'
    >
      <div className='flex justify-between'>
        <div className='w-[180px]'>
          <h1 className='text-base font-medium'>{name}</h1>
          <h2 className='text-[#C4C4C4] text-xs'>
            {formatDollar(participantNum)} Participants
          </h2>
        </div>

        <img
          src={picUrl}
          className='rounded-lg h-20 w-20 object-center object-cover'
        />
      </div>

      <div className='flex items-center gap-x-2 text-black text-xs font-medium'>
        {nft > 0 && <div className='px-1.5 bg-[#904BF6] rounded-full'>NFT</div>}
        {points > 0 && (
          <div className='px-1.5 bg-[#904BF6] rounded-full'>POINT</div>
        )}
      </div>
    </Link>
  )
}
