...
const TemplateComponent = lazy(() => import('../tokenTable/template'))
...
function PlanList () {
  ...
  useEffect(() => {
    if (swiper && filters.plan.length === 1 && tipList.length > 0) {
      const idx = tipList.findIndex(
        v => v.incentivePlanId === filters.plan[0]?.value
      )
      try {
  ...
  const getfilterGrantList = () => {
    ...
    if (status.length > 0) {
      res = res.filter(grant =>
        status.find(v => grant?.grant?.grantStatus === v.value)
      )
    }
    if (plan.length > 0) {
      res = res.filter(grant =>
        plan.find(v => grant?.grant?.incentivePlanId === v.value)
      )
    }
    if (vestingType.length > 0) {
      res = res.filter(grant =>
        vestingType.find(v => grant?.grant?.grantType === v.value)
      )
    }
    if (sortBy === 1) {
      // grantDate
      res = res.sort((a, b) =>
        dayjs(b?.grant?.grantDate, dateFormat).isBefore(
          dayjs(a?.grant?.grantDate, dateFormat)
        )
          ? -1
          : 1
      )
      ...
      res = res.map(v => ({
        ...v,
        lastestVest: getLastVested(v?.grant?.vestingSchedule?.vestingDetail)
          ?.date
      }))
      ...
      const vested = res
        ...
        .sort((a, b) =>
          dayjs(a.lastestVest, dateFormat).isBefore(
            dayjs(b.lastestVest, dateFormat)
          )
            ? 1
            : -1
        )
    ...
    <div className='w-full text-[#202124] mb-4 px-4 lg:px-0 lg:w-[936px] mx-auto'>
      ...
      >
        <div className='flex items-center justify-between mb-2 lg:mb-6'>
          <h2 className='font-bold text-ch2 lg:text-cwh1 dark:text-white'>
            Incentive Plans
          </h2>
            ...
            <>
              ...
              >
                {Array.isArray(tipList) &&
                  tipList.map(tip => {
                    const isActive =
                      filters?.plan?.length === 1 &&
                      filters?.plan[0]?.value === tip.incentivePlanId
        ...
        <div>
          <div className='flex items-center justify-between mb-2'>
            ...
            <button
              ...
              onClick={() =>
                navigate(`/incentive/grant/${filters.plan[0]?.value}/create`)
              }
              className={clsx(
        ...
        <div className='block lg:hidden'>
          ...
          <div
            className={clsx(
              ...
              filterGrantList.length > 0 ? 'grid-cols-2' : 'grid-cols-1'
            )}
          >
            {filterGrantList.length > 0 ? (
              filterGrantList.map(grant => (
