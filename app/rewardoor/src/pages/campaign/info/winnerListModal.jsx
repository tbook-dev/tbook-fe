...
export default function WhiteList ({ data, closeModal, open, modalType, per }) {
  // console.log({ data })
  const total = data?.filter(v => v.claimType === 4)?.length * per
  return (
      ...
      <div className='-mx-6 pt-6'>
        <h2 className='px-5 text-lg text-t-3 font-medium mb-4 flex items-center justify-between'>
          ...
          <span className='text-c-9 text-sm'>
            <span className='text-t-3 text-xl'>{formatDollar(total)}</span>
            {` ${modalType}${total > 1 ? 's' : ''} `}
            claimed
          </span>
        </h2>
