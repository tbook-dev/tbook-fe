...
export default function Settings() {
  ...
  const { project, projectId, address: currentUserAddress } = useUserInfo();
  ...
  const { data: adminData = [] } = useAdmins();
  const ownerAddress = adminData?.find?.((v) => v.isOwner)?.wallet;
  const isOwner = currentUserAddress === ownerAddress;
  const hasSocialData = useMemo(() => {
    return (
      [ownerDc?.connected, ownerTwitter?.connected].filter(Boolean).length > 0
    );
  }, [ownerDc, ownerTg, ownerTwitter]);
  // console.log({ ownerDc, ownerTg, ownerTwitter, hasSocialData });
