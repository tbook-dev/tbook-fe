...
function Avator({ setOpen, open, id }) {
  ...
  return (
    <div
      ...
      onClick={setOpen}
    >
      <div className={clsx("hidden lg:w-6 lg:h-6  lg:flex justify-center items-center mr-1")}>
        <NetWork id={id} />
      </div>
      <span className={clsx("hidden lg:block text-c1")}>{ensName || shortAddress(userStore?.mainWallet)}</span>
