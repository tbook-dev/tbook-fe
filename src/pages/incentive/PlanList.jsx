...
//                    if(existedList)
                  if (!existedList.find(item => item?.grant?.grantId === list?.[0]?.grant?.grantId) && !ignore) {
    ...
    return (
        <IncentiveLayout>
            <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
                            ...
                            >
                                <SwiperSlide>
                                    <NavLink to={"/incentive/create"}>
                                        <div
                                            className="w-[148px] h-[98px] shadow-c1 border rounded-[10px] flex justify-center align-middle">
                                            <PlusOutlined/>
                                        </div>
                                    </NavLink>
                                </SwiperSlide>
                                {tipList.map((tip) => {
                                    return (
                                        <SwiperSlide key={tip.incentivePlanId}>
                                            ...
                                            >
                                                <div
                                                    className="w-[148px] h-[98px] shadow-c2 border rounded-[10px] relative">
                                                    <div className="text-base text-[#3A4353] pt-3.5 pl-1.5">
                                                        <p>{tip.incentivePlanName}</p>
                                                    </div>

                                                    <div
                                                        className="absolute inset-x-0 h-1.5 overflow-hidden bottom-5 bg-[#CBD5E1]">
                                                        <div
                                                    ...
                                                    <div
                                                        className="inset-x-0 absolute bottom-0 origin-left	scale-50 text-[#1E293B]">
                                                        Granted {tip.grantedTokenNum}
                                                    </div>
                                                    <div
                                                        className="inset-x-0 absolute bottom-0 origin-right text-right	scale-50 text-[#1E293B]">
                ...
                <GrantTable list={grantList} title={() => <h2 className="font-bold text-base	inline">All The Grants <p
                    className="font-light	 inline">{grantList.length}</p></h2>
                }/>
